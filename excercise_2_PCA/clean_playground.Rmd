...
get_pcs_covering_variance <-function(data, searched_variance){
  ...
  #print(searched_pcs)
  df_pca[, seq(1, i, 1)]  # ERROR WITH THIS FUNCTION
  #df_pca[, 1:i]
}

# get the maximum needed number for getting the searched variance 
get_pca_number <-function(data, searched_variance){
  pca <- prcomp(data[,(2:ncol(data))], center = TRUE,scale. = TRUE)
  pca_sum <- summary(pca)
...
for (searched_accum_var in accumulated_variance_v){
  #wanted_pcs <- get_pcs_covering_variance(id, searched_accum_var)  # ERROR HERE
  limit_pca_rank <- get_pca_number(id, searched_accum_var)
  wanted_pcs <- pca <- prcomp(id[,(2:ncol(id))], center = TRUE,scale. = TRUE, rank. = limit_pca_rank)
  print(wanted_pcs)
  ...
  generateCipherFromPcaImages <- function(dataset, ciphers){
  ...
  generateCipherFromPcaImages(wanted_pcs, c(0:9))
