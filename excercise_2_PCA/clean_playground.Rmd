...
print_plots_pca <- function(data){
  ...
  plot(pca_std_variance, main="standard deviation of first 30 PCAs")
  
  pca_proportion_of_variance <- pca_sum$importance[2,1:30]
  plot(pca_proportion_of_variance, main="proportion of variance of first 30 PCAs")
  
  pca_cumulative_proportion <- pca_sum$importance[3,1:20]
  plot(pca_cumulative_proportion, main="cumulative proportion first 30 PCAs")
}
...
show_pca_reconstruction <- function(dataset, searched_variance){
  ...
  #}
  print(image(get_img_rot(restr, cipher_index), col = gray(0:100/100), xlab = paste("reconstructed with ", searched_accum_var, "% variance") ))
}
