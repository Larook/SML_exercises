...
get_plots_exc_2_1 <- function(id_dataset, is_disjunct){
  ...
  if (is_disjunct == TRUE){
      ...
      test_rows_id <- 1:(rows_per_person*floor(length(idList)/2))
      train_rows_id <- (rows_per_person*floor(length(idList)/2) + 1) : nrow(shuffled_df)
...
get_plots_exc_2_1(id_dataset=id, is_disjunct=TRUE)
```
