...
get_plots_exc_2_1 <- function(id_dataset, is_disjunct){
  set.seed(423)
  shuffled_df <- id_dataset[sample(nrow(id_dataset)),]  # or maybe just dont shuffle? shuffled_df <- id
  
  ks <- c(15, 35)# 75) #, 75) # ks <- c(5,9,13)
  ...
  for (searched_accum_var in accumulated_variance_v){
    ...
    for (k_now in ks){
      # split 50/50
      test_df <- wanted_pcs$x[1:(nrow(id)/2),]  # ? reconstructed_df
      train_df <- wanted_pcs$x[(nrow(id)/2 + 1):nrow(id),]
      
      # get the labels - supervision part
      id_test_labels <- shuffled_df[1:(nrow(id)/2), 1]
      id_train_labels <- shuffled_df[(nrow(id)/2 + 1):nrow(id), 1]
...
# SINGLE PERSON DATA
load("../data/id100.Rda")
get_plots_exc_2_1(id_dataset=id, is_disjunct=FALSE)
