...
knn_get_pca_plot_data <- function(test_data, test_labels, train_data, train_labels, repetitions = 10, k, searched_accum_var){
    ...
    for (rep in c(1:repetitions)){
      ...
      k_accuracy_test <-get_accuracy(id_test_labels, numbers_test_pred)
      ...
      k_accuracy_train <-get_accuracy(id_test_labels, numbers_test_pred)
...
plot_results <- function(plot_data_ks_l){
  ...
  #plot(ks, test_accuracy_means, type="l", ylab="test_accuracy_means", main=main)
  #plot(ks, train_accuracy_means, type="l", ylab="train_accuracy_means", main=main)
  ...
  g_test <- ggplot(df_plots, aes(x=ks)) + 
    geom_line(aes(y=test_accuracy_means)) + 
    geom_ribbon(aes(ymax=test_accuracy_means+test_accuracy_stds, ymin=test_accuracy_means-test_accuracy_stds), fill="pink", alpha=.5) +
    labs(title=main, 
         subtitle="test data accuracy", 
         caption="Source: abcd", 
         y="accuracy [%]",
         x="k-number of nearest neighbours")
  
  g_train <- ggplot(df_plots, aes(x=ks)) + 
    geom_line(aes(y=train_accuracy_means)) + 
    geom_ribbon(aes(ymax=train_accuracy_means+train_accuracy_stds, ymin=train_accuracy_means-train_accuracy_stds), fill="pink", alpha=.5) +
    labs(title=main, 
         subtitle="train data accuracy", 
         caption="Source: abcd", 
         y="accuracy [%]",
         x="k-number of nearest neighbours")
  
  list_of_plots <- list(g_time, g_test, g_train)
