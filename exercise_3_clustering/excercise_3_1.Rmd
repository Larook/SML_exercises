...
plot_results_cv_kmeans <- function(plot_data_ks_l){
  # TODO:
  # firstly we need to average among all the folds
  # probably just make it all to one dataframe
  # then select
...
we should just sum the equations from all the folds together and then divide/number of folds?
does number of folds in this case == number of people? I think yes, so if we use 2 people then 2 folds
```{r plot the data}
...
for (k in ks){
  ...
  for (i in 1:nrow(df_k3)){
    time_v_sum <- time_v_sum + unlist(df_k3$test_time_v[i])
    acc_v_sum <- acc_v_sum + unlist(df_k3$test_accuracy_v[i])
  }
  time_v_avg <- time_v_sum / nrow(df_k3)
  acc_v_avg <- acc_v_sum / nrow(df_k3)
}
